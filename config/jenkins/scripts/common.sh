...
ensure_runtime_defaults() {
  ...
  : "${REQUEST_CACHE_DEFAULT_TTL_SECONDS:=60}"
  : "${REDIS_HOST:=redis.${NAMESPACE}.svc.cluster.local}"
  : "${REDIS_PORT:=6379}"
  : "${REDIS_DB:=0}"
  : "${REDIS_KEY_PREFIX:=carepulse}"
  : "${REDIS_TLS_ENABLED:=false}"
  export NAMESPACE APP_PUBLIC_URL APP_INGRESS_HOST POSTGRES_PORT POSTGRES_DB POSTGRES_USER POSTGRES_SSLMODE DATABASE_MODE SSH_USER ENABLE_REQUEST_CACHE CACHE_PROVIDER REQUEST_CACHE_DEFAULT_TTL_SECONDS REDIS_HOST REDIS_PORT REDIS_DB REDIS_KEY_PREFIX REDIS_TLS_ENABLED
}
